...
def test_conflict_report_with_events(tmp_path):
    store = _store(tmp_path)
    f1 = WorldFact(
        domain="life", entity="king", attribute="alive", value=True, version=1, agent_id="agent-a"
    )
    store.set_fact(f1)
...
def test_conflicts_for_entity(tmp_path):
    store = _store(tmp_path)
    f = WorldFact(
        domain="life", entity="king", attribute="alive", value=True, version=1, agent_id="a"
    )
    store.set_fact(f)
    event = ContradictionEvent(
        rule="test_rule",
...
def test_conflicts_for_entity_filters_correctly(tmp_path):
    ...
    f_queen = WorldFact(
        domain="life", entity="queen", attribute="alive", value=True, version=1, agent_id="b"
    )
